...
本模块实现了基于 Soft Actor-Critic (SAC) 算法的 CTDE 智能体。

CTDE 架构:
    - 集中式训练: Critic 使用全局状态和联合动作
    - 分散式执行: Actor 仅使用局部观测和邻居信息
"""
import torch
...
class SACAgent:
    ...
    def __init__(self, topology, auto_entropy: bool = AUTO_ALPHA,
                 use_amp: bool = True) -> None:
        """初始化 SAC 智能体。

        Args:
            topology: DirectedSpanningTreeTopology 实例。
            auto_entropy: 是否启用自动熵系数调节。
            use_amp: 是否启用混合精度训练。
        """
        ...
        self.actor = DecentralizedActor(
            STATE_DIM, HIDDEN_DIM, use_neighbor_info=USE_NEIGHBOR_INFO
        ).to(DEVICE)
        ...
        if self.use_amp:
            ...
            print("CTDE-SAC with AMP enabled")
        else:
            self.scaler = None
            print("CTDE-SAC initialized")
    ...
    def _get_neighbor_obs(self, states: torch.Tensor,
                          batch_size: int) -> Tuple[Optional[torch.Tensor], Optional[torch.Tensor]]:
        """获取邻居观测和掩码。

        Args:
            states: 全局状态 (batch, num_agents, state_dim)。
            batch_size: 批次大小。

        Returns:
            neighbor_obs: (batch * num_followers, max_neighbors, state_dim)。
            neighbor_mask: (batch * num_followers, max_neighbors)。
        """
        if not self.use_neighbor_info:
            return None, None

        neighbor_obs = torch.zeros(
            batch_size, self.num_followers, self.max_neighbors, STATE_DIM, device=DEVICE
        )
        neighbor_mask = torch.zeros(
            batch_size, self.num_followers, self.max_neighbors, dtype=torch.bool, device=DEVICE
        )
        ...
        return neighbor_obs.view(-1, self.max_neighbors, STATE_DIM), neighbor_mask.view(-1, self.max_neighbors)

    def _get_follower_obs(self, states: torch.Tensor, batch_size: int) -> torch.Tensor:
        """提取跟随者观测。"""
        return states[:, 1:, :].reshape(-1, STATE_DIM)
    ...
    def select_action(self, state: torch.Tensor, deterministic: bool = False) -> torch.Tensor:
        ...
        Args:
            state: 当前状态，支持 (num_agents, state_dim) 或 (batch, num_agents, state_dim)。
    ...
    def update(self, batch_size: int = BATCH_SIZE,
               gradient_steps: int = GRADIENT_STEPS) -> Dict[str, float]:
        """更新网络参数。
        ...
        Returns:
            包含各项损失的字典。
        """
        if self.total_steps < self.warmup_steps or not self.buffer.is_ready(batch_size):
            return {}

        total_q1_loss, total_q2_loss, total_actor_loss = 0, 0, 0
